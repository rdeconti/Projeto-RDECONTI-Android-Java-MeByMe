...
public class ModuleFragmentChallengeGraphAnalysis extends Fragment {
    ...
    // *********************************************************************************************
    // *********************************************************************************************
    public ModuleFragmentChallengeGraphAnalysis() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup viewGroup,
        ...
        mView = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.challenge_graph_fragment, viewGroup, false);

        initializeLayoutData();

        SqliteModuleChallengeAnswer.sqliteGraphAnalysis(mContext);
    ...
    private void generatePieChart() {
        ...
        if (value1 > INT_NUMBER_00) {
            pieEntries.add(new PieEntry(value1, getString(R.string.label_status_new), 1));
        }

        if (value2 > INT_NUMBER_00) {
            pieEntries.add(new PieEntry(value2, getString(R.string.label_status_pending), 2));
        }

        if (value3 > INT_NUMBER_00) {
            pieEntries.add(new PieEntry(value3, getString(R.string.label_status_closed), 3));
        }

        if (value4 > INT_NUMBER_00) {
            pieEntries.add(new PieEntry(value4, getString(R.string.label_status_ignored), 4));
        }

        if (value5 > INT_NUMBER_00) {
            pieEntries.add(new PieEntry(value5, getString(R.string.label_status_postponed), 5));
        }
    ...
    private void generateHorizontalBarChart() {
        ...
        final String[] mTableLabels = new String[]
                {   mContext.getString(R.string.label_status_new),
                        mContext.getString(R.string.label_status_pending),
                        mContext.getString(R.string.label_status_closed),
                        mContext.getString(R.string.label_status_ignored),
                        mContext.getString(R.string.label_status_postponed)};
